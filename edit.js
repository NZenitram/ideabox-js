...
function editClickTitle() {
  ...
  for (var i = 0; i < title.length; i++) {
    ...
    title[i].addEventListener('blur', editTitle)
  }
}
...
function editTitle(e){
  ...
  if (e.which == 13 || e.type == 'blur') {
    ...
    var idea = this.parentElement.children[1].innerText
  }
  search(idea, ideas, newContent)
}
...
function search(nameKey, localStorage, newContent){
  for (var i = 0; i < localStorage.length; i++) {
    ...
    } else if (localStorage[i].idea === nameKey) {
      updateLocalIdea(localStorage[i], localStorage, newContent)
    }
  }
}
...
function updateLocalIdea(idea, newIdeas, newContent){
  idea.title = newContent
  localStorage.setItem('ideabox', JSON.stringify(newIdeas))
  clearList()
}
